...
async def lifespan(app: FastAPI):
    ...
    try:
        import asyncio
        # Add timeout - fail after 10 seconds instead of hanging forever
        await asyncio.wait_for(
            MCPClientManager.get_client(mcp_server_path),
            timeout=10.0
        )
        ...
        print("⚠️  Make sure Joshna's data server is running on port 5000")
        # Don't raise - allow server to start even if MCP isn't ready
        print("⚠️  Server starting anyway - MCP will be unavailable")
